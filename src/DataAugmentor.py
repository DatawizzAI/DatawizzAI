...
class DataAugmentor:
    def __init__(self, llm, structure='', batch_size=10, verbose=True):
        ...
        data_augmentor_template = (
            ...
            f"for the data described below. In the following structure, but with different values."
        ...
        self.data_augmentor_chain = LLMChain(
            llm=llm,
            prompt=data_augmentor_prompt,
            verbose=verbose,
        )
    ...
    def generate_data(self, query=None, region=None, language=None, task_specifications=None, num_records=3, leading_key='', max_retries=3,
                    total_max_retries=10):
        """
            ...
            while n < num_records:
                ...
                while retries < max_retries:
                    try:
                        ...
                        res = self.data_augmentor_chain.predict(structure=cur_structure, randomness=random.random(),
                                                               human_input=query_msg,previous_generated_batch=self.previous_generated_batch)
                        self.previous_generated_batch = res
                        results_dict = parse_output(res, results_dict)
                        if leading_key == '':
                            leading_key = list(results_dict.keys())[0]
                        self.leading_key = leading_key
                        n = results_dict[leading_key].shape[0]
    ...
    async def generate_data_in_parallel(self, records, region=None, language=None, task_specifications=None, query='', max_retries=3):
        """
        ...
        iterations = math.ceil(records / self.batch_size)
        ...
        for key in results.keys():
            results[key] = results[key][:records]
